...
if __name__=='__main__':
    ...
    ##定义优化器与损失函数
    optimizer = optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=cfg.LR)       #filter(lambda实例
    # optimizer = optim.Adam(model.parameters(), lr=cfg.LR)
    # optimizer = optim.SGD(model.parameters(), lr=cfg.LR,
    #                       momentum=cfg.MOMENTUM, weight_decay=cfg.WEIGHT_DECAY)

    # ll = [11, 22, 33, 44, 55]          filter(lambda示例
    ...
    for iteration in range(start_iter, max_iter):
        ...
        if iteration % epoch_size == 0:
            ...
            batch_iterator = iter(train_dataloader)       #iter()函数和next()函数用于迭代  共用
            loss = 0
            epoch += 1
            ###保存模型
        ...
        prediction = torch.max(out, 1)[1]      #torch.max(a,1)返回每一行中最大值的那个元素，且返回其索引（返回最大元素在这一行的列索引）
        ...
        x1 = range(0, iteration + 1)
        x2 = range(0, iteration + 1)
        y1 = train_acc_list
        y2 = Loss_list
        plt.subplot(2,1,1)
        plt.plot(x1, y1, '.-')

        plt.xlabel('accuary vs. iteration')
        plt.ylabel('accuary')
        plt.subplot(2,1,2)
        plt.plot(x2, y2, '.-')
        plt.xlabel('loss vs. iteration')
        plt.ylabel('loss')
        if(iteration%10==0):
            plt.savefig("accuary.jpg")
            plt.show()
